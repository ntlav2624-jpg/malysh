...
class MalyshBioThermalV19:
    def __init__(self, x_size=4, y_size=4, z_layers=3, wal="malysh_v19_wal.log"):
        self.x_size = x_size
        self.y_size = y_size
            ...
            for v in self.graph.nodes():
                if u < v:
                    ...
                    if dist <= 1 or (dist == 2 and uz != vz):
                        ...
                        self.graph.add_edge(u, v, weight=weight, flow=0.0)

    def recover_from_wal(self):
        if not os.path.exists(self.wal_file):
            print(f"[BIOTHERMAL V19] Холодный старт био-термального ядра.")
            return False
        try:
            ...
            print(f"[BIOTHERMAL V19] Восстановление из WAL на такте: {self.tick}")
            return True
        except Exception as e:
            print(f"[BIOTHERMAL V19] Ошибка чтения WAL: {e}")
            return False

    def process_spiral_and_metabolism(self, val, spiral_points):
        for node in self.graph.nodes():
            self.graph.nodes[node]['is_spiral_core'] = False
        ...
        for sx, sy, sz in spiral_points:
            for node, data in self.graph.nodes(data=True):
                ...
                if nx_x == sx and nx_y == sy and nx_z == sz:
                    ...
                    data['is_spiral_core'] = True
                    # Спираль инжектирует энергию прямо в воксельный объём
                    data['energy'] = min(100.0, data['energy'] + 45.0)
                    data['temperature'] += 3.0

        # Спираль управляет потоками и проводимостью графа
        for u, v, data in self.graph.edges(data=True):
            if u in active_spiral_nodes or v in active_spiral_nodes:
                data['weight'] = max(0.05, data['weight'] - 0.12)
                data['flow'] += 1.0
            else:
                data['weight'] = min(3.0, data['weight'] + 0.015)

        # Термодинамика узлов и переходы Base-4
        for node in self.graph.nodes():
            n_data = self.graph.nodes[node]
            if n_data['energy'] > 10.0 and n_data['temperature'] < 97.0:
                n_data['energy'] -= 3.5
                n_data['temperature'] += 1.4
        ...
        # Маршрутизация через управляемые ребра
        for z in range(self.z_layers):
            ...
            if len(sub_nodes) > 1:
                ...
                if sources:
                    ...
                    target = sources[(self.tick + 1) % len(sources)]
                    if src != target:
                        try:
                            if nx.has_path(sub_g, src, target):
                                ...
                                for i in range(len(path) - 1):
                                    ...
                                    if self.graph.has_edge(u, v):
                                        self.graph[u][v]['weight'] = max(0.08, self.graph[u][v]['weight'] - 0.04)
                        except Exception:
                            pass
    ...
    def render_dynamic_voxel_frame(self, cam_angle):
        ...
        # 3. Энергия и температура узлов прямо влияют на рендер и палитру
        for node, data in self.graph.nodes(data=True):
            x, y, z = data['xyzt'][:3]
            ...
            is_spiral = data['is_spiral_core']

            if is_spiral:
                color_array[x, y, z] = '#FF4500' # Эпицентр спирального потока (Огненно-оранжевый)
            elif temp > 82.0:
                color_array[x, y, z] = '#C70039' # Перегретый критический узел
            elif energy > 70.0:
                color_array[x, y, z] = '#2ECC71' # Высокий энергетический потенциал (Зеленый)
            elif energy < 25.0:
                color_array[x, y, z] = '#F1C40F' # Истощенный узел (Желтый)
            else:
                color_array[x, y, z] = '#2471A3' # Стабильное состояние (Синий)

        fig = plt.figure(figsize=(6, 6))
        ax = fig.add_subplot(projection='3d')
        ax.voxels(voxel_array, facecolors=color_array, edgecolor='black')
        
        # Камера следует по законам логарифмической спирали
        elevation = 30 + 15 * math.sin(cam_angle * 0.45)
        azimuth = math.degrees(cam_angle) % 360
        ax.view_init(elev=elevation, azim=azimuth)

        ax.set_title(f"Bio-Thermal V19 [Tick: {self.tick:03d}]")
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.set_zlabel("Z-Layer")
        
        filename = f"v19_frame_{self.tick:03d}.png"
        plt.savefig(filename, dpi=110)
        plt.close(fig)
        self.frame_files.append(filename)
        print(f"[BIOTHERMAL V19] Отрендерен термо-объём: {filename}")

    def compile_animation(self):
        if not self.frame_files:
            return
        print("[BIOTHERMAL V19] Сборка био-термальной анимации...")
        images = [Image.open(f) for f in self.frame_files]
        gif_name = "malysh_biothermal_continuum.gif"
        images[0].save(
            gif_name,
            save_all=True,
        ...
        print(f"[BIOTHERMAL V19] Анимация сохранена: {gif_name}")
    ...
    def run_engine(self, stream):
        ...
        for val in stream:
            ...
            # Расчет точек спирали в пространстве
            spiral_points = []
            for d_t in range(3):
                s_angle = (self.tick - d_t) * 0.42
            ...
            self.process_spiral_and_metabolism(val, spiral_points)
            self.render_dynamic_voxel_frame(self.tick * 0.42)
        ...
        print(f"\n[BIOTHERMAL V19] Цикл завершен. Тактов: {self.tick}. WAL синхронизирован.")

if __name__ == "__main__":
    engine = MalyshBioThermalV19()
